...
Компилятор обитает в папках `"C:\Program Files (x86)\Microsoft SDKs\F#\10.1\Framework\v4.0"` и `"C:\Program Files (x86)\Microsoft SDKs\F#\4.0\Framework\v4.0"`.

Опции вызова компилятора следующие:
...
--platform:<string>            Limit which platforms this code can run on: x86, Itanium, x64, anycpu32bitpreferred, or anycpu. The default is anycpu.
--nooptimizationdata           Only include optimization information essential for implementing inlined constructs. Inhibits cross-module inlining but improves binary compatibility.
